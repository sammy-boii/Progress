...
def draw(targets):
    ...
    for target in targets:
        target.draw()   #   calling the method of target class on each target
...
# when an event occurs it is stored in an event queue. And in the event loop it checks the events that has occured since it was last called. 